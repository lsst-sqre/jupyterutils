...
def parse_args(cfg=LSSTConfig(),
    ...
    with start_action(action_type="parse_args"):
        ...
        if component not in allowed_components:
            raise ValueError(
                "Component {} not in {}!".format(component,
                                                 allowed_components))
        ...
        parser.add_argument("-d", "--debug", action="store_true",
                            ...
                            default=dbg)
        parser.add_argument("-f", "--cachefile",
        ...
        parser.add_argument("-n", "--name",
                            ...
                            default=lrn)
        parser.add_argument("-q", "--dailies", "--daily", "--quotidian",
                            type=int,
        ...
        parser.add_argument("-e", "--experimentals", "--experimental",
                            "--exp", type=int,
                            help=(
                                "# of experimental builds to keep [{}]".format(
                                    kex)),
                            default=kex)
        if component != "reaper":
            parser.add_argument("-b", "--releases", "--release", type=int,
                                help=(
                                    "# of release builds to keep [{}]".format(
                                        krl)),
                                default=krl)
        ...
        if component == "prepuller":
            ...
            parser.add_argument("--command",
                                help=("JSON representation of command " +
        ...
        if (hasattr(results, "command") and results.command and
                type(results.command) is str):
